	...
	private static Element buildXmlModel(WebServiceRequest webService) throws RequestFactoryException {
		try {
			...
			if (webService.getWebServiceRequestModel() == WebServiceRequestModel.CompositeRequest) {
				...
				//NestIT-ISO-186
				if(request.getDataRows() != null && request.getDataRows().length > 0) {
					for (int i=0; i < request.getDataRows().length; i++  ) {
						if (request.getDataRows()[i].getFieldsCount() > 0) {
							xmlModel.appendChild(doc.importNode(buildXmlFieldsContainer(request.getDataRows()[i]), true));
						}
					}
				}

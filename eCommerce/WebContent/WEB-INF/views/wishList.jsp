							...
							<tbody>
								<tr
									...
									<a href="#/product/{{item.product.productID}}"> {{item.product.name}} </a></td>
																	<!-- <td>{{item.quantity}}</td> -->
									<td><input type="Number" ng-model="item.quantity" value="{{item.quantity}}"  min="1" max="99" /></td>
									<td>{{item.product.priceList}}</td>
									<td>$0.00</td>
									<td><input class="form-control total-price" id="ex1" type="text" value="{{item.product.priceList * item.quantity}}" readonly="readonly"/></td>
									 <td><a href="" ng-click="deleteProductFromList(item.wishID,item.product.productID,item.userId,$index)"><i class="fa fa-trash-o"></i></a>
